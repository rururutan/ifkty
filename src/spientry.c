 ...
 IsSupportedW(LPCWSTR filename, void* variant)
{
	...
	length = WideCharToMultiByte(CP_ACP, 0, filename, -1,
	                             NULL, 0, NULL, NULL);
	if (length <= 0) return FALSE;
	narrow = (LPSTR)LocalAlloc(LMEM_FIXED, (SIZE_T)length);
	if (narrow == NULL) return FALSE;
	if (WideCharToMultiByte(CP_ACP, 0, filename, -1,
	                        narrow, length, NULL, NULL) == 0) {
		LocalFree((HLOCAL)narrow);
		return FALSE;
	}
	supported = IsSupported(narrow, variant);
	LocalFree((HLOCAL)narrow);
	return supported;
}
